...
def split_dataset(data_df):
    '''
        ...
        Input: trainning - testing dataframe
        Output: X_data - X_target
    '''
    X_data  = data_df.drop(['WithinLimits', 'OrderNo'], axis = 1)
    y_target  = data_df['WithinLimits']
...
data_df = pd.read_csv("../data/Training_Full_Data.csv")
X_data, y_target = split_dataset(data_df)
X_tsne = reduce_dimension(X_data)
...
distributions = [
    ('Unscaled data', X_tsne),
    ('Data after standard scaling',
        reduce_dimension(StandardScaler().fit_transform(X))),
    ('Data after min-max scaling',
        reduce_dimension(MinMaxScaler().fit_transform(X))),
    ('Data after max-abs scaling',
        reduce_dimension(MaxAbsScaler().fit_transform(X))),
    ('Data after robust scaling',
        reduce_dimension(RobustScaler(quantile_range=(25, 75)).fit_transform(X))),
    ('Data after quantile transformation (uniform pdf)',
        reduce_dimension(QuantileTransformer(output_distribution='uniform')
        .fit_transform(X))),
    ('Data after quantile transformation (gaussian pdf)',
        reduce_dimension(QuantileTransformer(output_distribution='normal')
        .fit_transform(X))),
    ('Data after sample-wise L2 normalizing',
        reduce_dimension(Normalizer().fit_transform(X)))
]
